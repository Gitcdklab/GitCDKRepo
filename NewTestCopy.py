...
def main():
    ...
    SOURCE_FILE_KEY = get_arg("SOURCE_FILE_KEY")           # e.g., input/abc.txt
    BASE_NAME     = get_arg("BASE_NAME")                   # override base (no extension)
    ...
    # - If SRC_KEY is a folder, find the single data object inside.
    # - If SRC_KEY is a file, use it directly.
    if SRC_KEY.endswith("/"):
        data_key = find_single_data_key_under_folder(BUCKET, SRC_KEY)
        folder_prefix = SRC_KEY.rstrip("/") + "/"
        print(f"[INFO] Resolved single data object under folder: s3://{BUCKET}/{data_key}")
    else:
        data_key = SRC_KEY
        folder_prefix = os.path.dirname(SRC_KEY.rstrip("/")) + "/"

    # Derive base name from SOURCE_FILE_KEY (preferred) or BASE_NAME override or fallback to data_key
    if BASE_NAME:
        base_no_ext = BASE_NAME  
        src_ext = ""             
    elif SOURCE_FILE_KEY:
        src_name, src_ext = source_base_from_key(SOURCE_FILE_KEY)
        base_no_ext = src_name
    else:
        src_name, src_ext = source_base_from_key(data_key)
        base_no_ext = src_name
    ...
    print(f"[INFO] Moving s3://{BUCKET}/{data_key} -> s3://{BUCKET}/{dst_key} (OUT_EXT_MODE={OUT_EXT_MODE})")
    ...
    try:
        ...
        '''# Optional: delete _SUCCESS in the source folder if it exists and source was a folder
        if SRC_KEY.endswith("/") and DELETE_SUCCESS:
